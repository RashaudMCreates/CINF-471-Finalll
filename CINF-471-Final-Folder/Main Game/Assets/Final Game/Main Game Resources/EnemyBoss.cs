...
public class EnemyBoss : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (timer >= 4f)
        {
            anim.Play("BossEnemyFlury");
            timer = 0;
        }

        if (timer2 >= 6f)
        {
            anim.Play("EnemyBossUppper");
            timer2 = 0;
        }
    ...
    private void OnCollisionEnter(Collision collision)
    {
        ...
        if (collision.contacts[0].thisCollider.gameObject == EnemyRightFist)
        {
            ...
            if (collision.collider.GetComponent<Player>() != null)
            {
                ...
                SetHealth2(-60f); // or whatever action you want
            }
            if (Health <= 0)
            {
                LaunchEnemy(enemyRb);
        ...
        if (collision.contacts[0].thisCollider.gameObject == EnemyLeftFist)
        {
            ...
            if (collision.collider.GetComponent<Player>() != null)
            {
                ContactHit.Play();
                Debug.Log("RightFist hit a player object HARD!");
                SetHealth2(-70f); // or whatever action you want
            }
            if (Health <= 0)
            {
                LaunchEnemy(enemyRb);
